...
SELECT MAX(stock_quantity) FROM t_shirts WHERE brand = 'Nike';  #3

SELECT SUM(price * stock_quantity * 0.95) FROM t_shirts WHERE brand = 'Adidas'; #2

SELECT SUM(stock_quantity) FROM t_shirts WHERE brand = 'Adidas' AND color = 'black' AND size = 'XS'; #6

SELECT SUM(stock_quantity) FROM t_shirts WHERE brand = 'Nike' AND size = 'XS' AND color = 'white';  #1

SELECT SUM(price * stock_quantity) FROM t_shirts WHERE brand = 'Levi'   AND NOT color = 'Black'   AND NOT size IN ('3XL', '4XL');  #4

SELECT SUM(price * stock_quantity)  FROM t_shirts  WHERE brand = 'Levi' AND stock_quantity > 0; #5

SELECT SUM(price * stock_quantity) FROM t_shirts WHERE brand = 'Nike' AND NOT color = 'white' AND NOT size IN ('XL', 'XXL');  #7

SELECT MAX(stock_quantity) FROM t_shirts WHERE brand = 'Adidas'; #8
  SELECT COUNT(*) FROM t_shirts WHERE brand = 'Nike' AND stock_quantity > 0;
   SELECT SUM(price * stock_quantity) FROM t_shirts WHERE brand = 'Levi' AND stock_quantity > 0;

 select * from t_shirts where brand = 'Nike' and color = 'Black';
 SELECT COUNT(*) FROM t_shirts WHERE brand = 'Nike' AND stock_quantity > 0;
 SELECT SUM(price * stock_quantity) FROM t_shirts WHERE brand = 'Levi' AND stock_quantity > 0;
 
SELECT SUM(stock_quantity) FROM t_shirts WHERE brand = 'Nike' AND size = 'L' AND color = 'Black';